...
int INPUT_MIN = 20;  // 動き出すギリギリのinput
int INPUT_MAX = 128;
int INPUT_START = 30;  // 初動input
...
int pidCalc(double speed_id) {
  if (speed_id == 0) {
    ...
    } else if (rotation_count <= 2) {  // 1,2周目はinputを固定
      input = (int)(speed_id / K_CAR);
    } else {  // 3周目以降はe0,e1が使える
      input = (int)(kp*e0 + ki*I + kd*(e0-e1)/dt)+ INPUT_MIN;
    }
    input = min(input, INPUT_MAX);
    input = max(input, INPUT_MIN);
    println(getTime(), "[PID] rc oldt newt dt speed e1 e0 input", rotation_count, old_time, new_time, dt, speed, e1, e0, input);
    return input;
  }
}

void stopCheck() {
  if (rotation_count > 0 && millis()/1000 - new_time > (2*PI*r)/SPEED_MIN) {
